...
contract TokenValidator is ITokenValidator, IUniswapV2Callee, ImmutableState {
    ...
    function _validate(
        ...
        address pairAddress = SmartRouterHelper.pairFor(
            this.factoryV2(),
            token,
            baseToken
        );
        ...
        (, bytes memory returnData) = address(pairAddress).call(
        ...
        try
            pair.swap(
                ...
                amount1Out,
                address(this),
                abi.encode(balanceBeforeLoan, amountToBorrow)
            )
    ...
    function isTransferFailed(
        ...
        assembly {
            ...
            transferFailed := eq(
                keccak256(ptr, suffixLength),
                keccak256(suffixPtr, suffixLength)
            )
        }
    ...
    function uniswapV2Call(
        ...
        (uint256 balanceBeforeLoan, uint256 amountRequestedToBorrow) = abi
            .decode(data, (uint256, uint256));
        uint256 amountBorrowed = tokenBorrowed.balanceOf(address(this)) -
            balanceBeforeLoan;
