...
/// @title Canonical XVerseSwap V3 factory
/// @notice Deploys XVerseSwap V3 pools and manages ownership and control over pool protocol fees
contract XVerseV3Factory is IXVerseV3Factory {
    /// @inheritdoc IXVerseV3Factory
    ...
    function createPool(
        ...
        pool = IXVerseV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IXVerseV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IXVerseV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IXVerseV3Pool(pool).setLmPool(lmPool);
    }
}
