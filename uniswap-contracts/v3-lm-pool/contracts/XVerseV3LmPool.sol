...
import "@xverseprojects/v3-core/contracts/libraries/FixedPoint128.sol";
import "@xverseprojects/v3-core/contracts/interfaces/IXVerseV3Pool.sol";

import "./libraries/LmTick.sol";

import "./interfaces/IXVerseV3LmPool.sol";
import "./interfaces/IMasterChefV3.sol";

contract XVerseV3LmPool is IXVerseV3LmPool {
    ...
    IXVerseV3Pool public immutable pool;
    ...
    modifier onlyPoolOrMasterChef() {
        require(
            msg.sender == address(pool) || msg.sender == address(masterChef),
            "Not pool or MC"
        );
        _;
    }
    ...
    ) external override onlyPoolOrMasterChef {
        if (currTimestamp <= lastRewardTimestamp) {
            return;
        }

        if (lmLiquidity != 0) {
            (uint256 rewardPerSecond, uint256 endTime) = masterChef
